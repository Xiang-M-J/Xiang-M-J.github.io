function addKeyListeners() {
	...
	keypress.register_combo({
		...
		keys: "down",
		on_keydown: function() {
			const tempSpeed = settings.speedModifier;
			if (MainHex && gameState !== 0){
	...
	// $("#rows").onchange = function (ev){
	// 	console.log(ev.target())
	// }
	$("#rows").on("change", function () {
		let index = this.options.selectedIndex;
		settings.rows = this.options[index].value;
		// console.log(this)
		this.blur();
	})
	...
	window.colors = ["#8e44ad", "#f1c40f", "#065279", "#d35400"];

	window.hexColorsToTintedColors = {
		"#8e44ad": "rgb(229,152,102)",
		"#f1c40f": "rgb(246,223,133)",
		"#5bb9ff": "rgb(91,185,255)",
...
function handleClickTap(x,y) {
	...
	const radius = settings.hexWidth;
