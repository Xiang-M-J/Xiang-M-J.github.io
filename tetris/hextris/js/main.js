function scaleCanvas() {
	...
	if (window.devicePixelRatio) {
		const cw = $("#canvas").attr('width');
		const ch = $("#canvas").attr('height');
...
function setBottomContainer() {
	const buttonOffset = $("#buttonCont").offset().top;
	const playOffset = trueCanvas.height / 2 + 100 * settings.scale;
	const delta = buttonOffset - playOffset - 29;
	if (delta < 0) {
...
function animLoop() {
	let now;
	let dt;switch (gameState) {
	case 1:
		requestAnimFrame(animLoop);
		render();
		now = Date.now();
		dt = (now - lastTime) / 16.666 * rush;
		if (spd > 1) {
			dt *= spd;
		}
